//In this component TweetScreen The basic function of this component is to display a single tweet with its details,
// allow the user to create a reply to the tweet, and display the replies to that tweet.
// To make this it fetches the tweet data and replied tweets data from the backend using Redux actions.
...
const TweetScreen = ({ history, match }) => {
	...
	//the useSelector hook is used to get information from the Redux store related to the user, tweet, and related data
	// such as liked/unliked, retweeted/unretweeted, bookmarked/unbookmarked tweets.
	...
	// useEffect hook perform actions that are dispatched based on conditions.
	// If the user is not logged in, the user is redirected to the login page.
	// If the user is logged in and the user info and tweet data is not available,
	// they are fetched from the server using the corresponding actions.
	useEffect(() => {
		if (!userId) {
			history.push('/login')
		} else {
	...
	// Function uploadFileHandler which handles uploading an image for the tweet using the Axios to make a POST request to a backend
	// API endpoint that accepts multipart/form-data.
	...
	//here we iterates through the replied tweets data to match the user data of each tweet and associate it with the tweet data.
	...
	return (
		<>	
			{/*if homeLoading is true and  it displays a full-screen loader.
			 Then, it renders a div element with a container class that contains the tweet screen's content. */}

			{homeLoading && <FullScreenLoader />}
			<div className='container '>
				{/* modal  */}

				{/* userInfo exists, it displays a TweetModal component that allows the user to create a new tweet.*/}
