//The ProfileScreen component that displays the profile page of a user .
// It retrieves data from the Redux store using useSelector() hooks and dispatches actions using useDispatch() hook to retrieve tweets
// liked tweets, retweeted tweets, and user information, including the user's profile picture, name, handle, and bio.
...
const ProfileScreen = ({ history }) => {
	//useSelector() hooks to extract data from the Redux store
	// hooks subscribe the component to the relevant slice of state so that it will re-render when the state changes.
	...
	// useEffect() hook handles side effects, such as fetching user information, tweets, liked tweets, and retweeted tweets server.
	// The useEffect() hook runs on change in any of the dependencies here when the history, dispatch, userInfo, userId, unliked,
	// liked, ret, del, unret, bTweet, and unbTweet is changed.
	useEffect(() => {
		if (!userId) {
			history.push('/login')
		}
	...
	//logic to add user data to the retweeted and liked tweets data so that the data can be displayed correctly.
	//We have iterated over the retweetedTweets and likedTweets arrays and added the userdata property to the tweet object based
	// on the user ID.
	if (retweetedTweets) {
	...
	//deleteTweetByID dispatches the tweetDelete() action creator when called to delete a tweet.
	...
	return (
		<>
			{/* check for loading, on true,a FullScreenLoader component is rendered to indicate that data is being loaded.
			 Then check for errors in retrieving liked and retweeted tweets from the database and dispatch the appropriate actions
			  if an error is detected. */}
			{loading && <FullScreenLoader />}
			<div className='container'>
				{errorLiked && dispatch(getLikedTweets(userId._id))}
				{errorRetweeted && dispatch(getRetweetedTweets(userId._id))}
				{/* userInfo (user information),is passed down as a prop to the component.
				 If userInfo is present, a TweetModal component is rendered, which displays a modal window for composing and 
				 sending tweets.  */}
							...
							<div
								...
								<div className=' pt-0 profile'>
									...
									<div
										...
										style={{
											backgroundImage: `url(uploads/${userInfo.coverPhoto.split('uploads')[1]
												})`,
											backgroundRepeat: 'no-repeat',
											backgroundPosition: `${userInfo.posi}`,
											backgroundSize: 'cover',
										}}></div>
									{/* User profile image and edit option button	 */}
