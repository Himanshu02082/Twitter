//The HomeScreen component is responsible for rendering the home screen of the app, which displays a user's timeline
// of tweets from the people they follow.
...
const HomeScreen = ({ history }) => {
	...
	//HomeSreen uses many different Redux action creators and selectors, which are used to manage the state of the application.
	//The state is managed using Redux and accessed using the useSelector hook, and actions are dispatched using the useDispatch hook.
	...
	//useEffect hook is used to trigger side effects here the useEffect hook is used to check if the user is logged in
	// and fetch the tweets from users that the logged-in user is following.
	useEffect(() => {
		if (!userId) {
			history.push('/login')
		} else {
	...
	// such as the text and image of the tweet being created, and whether an image is currently being uploaded.
	const [text, setText] = useState('')
	const [image, setImage] = useState('')
	const [imageLoading, setImageLoading] = useState(false)

	//These functions made below are used to handle various events, such as creating a new tweet, uploading an image,
	// and deleting a tweet.
	...
	//here we are iterating over the tweets and users,while matching the user field of each tweet object with the _id field
	// of each user object. When a match is found, it is adding a new userdata field to the tweet object and assigning
	// it the corresponding user object.
	if (followingTweet) {
	...
	return (
		<>
			{/* full-screen loader component that appears when the data is being fetched in the background. */}
			{homeLoading && <FullScreenLoader />}
			{/* A header component that displays the title of the home screen. */}
			<Header title='Home' />
			<div className='container '>
				{/* modal component that allows the user to create a new tweet.*/}

				{userInfo && <TweetModal userInfo={userInfo} />}

				{/* modal ends */}

				{/* navigation section containing a side navigation bar and a logout button.
				 This section is only visible when the screen size is medium to large.
				  The side navigation bar displays the user's profile photo, username, and a list of navigation items. */}
				{userInfo && (
					<div className='row '>
						...
						{/* tweet section that contains the main content of the home screen.
						 This section displays the user's profile photo, a text area where the user can enter a new tweet,
						  and a list of tweets from other users that the current user is following. */}
						<div
							id='tweets-section'
							onScroll={scroll}
